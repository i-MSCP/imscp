...
Before running the i-MSCP installer for upgrading your installation to this new version, you must stop the
`imscp_panel` service manually.
...
for authenticating to AWStats interfaces of your domains and subdomains. Right now it is mandatory to always use your
customer account name (which is the main domain name) to authenticate. However, you can still change the password for
that user through the protected area management interface.

### i-MSCP frontEnd (imscp_panel service)

The i-MSCP frontEnd is now run through its own (isolated) PHP FPM daemon instance. Previously, the frontEnd was run
through PHP CGI with spawn-fcgi.
...
This parameter allows the administrator to enable or disable the PHP opcode cache for the customers. Default value is
`1` (enabled).

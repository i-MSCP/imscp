...
<table style="border-color: #000000; border-style: solid; border-width: 1px; padding: 5px;">
	<tr><td>
		<table border="0" cellspacing="2" cellpadding="2" style="margin-<?php echo __("left"); ?>: 5px;">
			<tr>
				<td rowspan="4" style="vertical-align: top;"><input type="button" class="extralongbutton" value="Set all permissions" onclick="CopyCheckboxToAll(document.ChmodForm, 'header[owner_read]', 'owner_read'); CopyCheckboxToAll(document.ChmodForm, 'header[owner_write]', 'owner_write'); CopyCheckboxToAll(document.ChmodForm, 'header[owner_execute]', 'owner_execute'); CopyCheckboxToAll(document.ChmodForm, 'header[group_read]', 'group_read'); CopyCheckboxToAll(document.ChmodForm, 'header[group_write]', 'group_write'); CopyCheckboxToAll(document.ChmodForm, 'header[group_execute]', 'group_execute'); CopyCheckboxToAll(document.ChmodForm, 'header[other_read]', 'other_read'); CopyCheckboxToAll(document.ChmodForm, 'header[other_write]', 'other_write'); CopyCheckboxToAll(document.ChmodForm, 'header[other_execute]', 'other_execute'); update_input(); " /></td>
			</tr>
			<tr>
				<td>Owner:</td>
		...
		<div style="font-size: 65%">To set all permissions to the same values, enter those permissions above and click on the button "Set all permissions"</div>
	</td></tr>
</table><br />

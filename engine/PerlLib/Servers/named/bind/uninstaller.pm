...
# Copyright (C) 2010-2019 by Laurente Declercq <l.declercq@nuxwin.com>
#
...
sub _removeConfig
{
    ...
    try {
        ...
        for my $file ( 'BIND_CONF_FILE', 'BIND_LOCAL_CONF_FILE', 'BIND_OPTIONS_CONF_FILE' ) {
            ...
            my $rs = iMSCP::File->new( filename => "$self->{'bkpDir'}/$filename.system" )->copyFile( $self->{'config'}->{$file}, { preserve => 'no' } );
            return $rs if $rs;

...
echo "\$::questions{'SERVER_HOSTNAME'} = 'vagrant.i-mscp.net';" >> /tmp/preseed.pl
echo "\$::questions{'BASE_SERVER_VHOST'} = 'panel.vagrant.i-mscp.net';" >> /tmp/preseed.pl
...
echo "\$::questions{'BASE_SERVER_IP'} = \$serverIps[0];" >> /tmp/preseed.pl
echo "\$::questions{'BASE_SERVER_PUBLIC_IP'} = \$serverIps[0];" >> /tmp/preseed.pl

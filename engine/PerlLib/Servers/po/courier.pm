...
sub addMail
{
	...
	if($data->{'MAIL_TYPE'} =~ /_mail/) {
		...
		my $uid = scalar getpwnam($mta->{'postfixConfig'}->{'MTA_MAILBOX_UID_NAME'});
		my $gid = scalar getgrnam($mta->{'postfixConfig'}->{'MTA_MAILBOX_GID_NAME'});
		my $mailDir = $mta->{'postfixConfig'}->{'MTA_VIRTUAL_MAIL_DIR'};
...
sub postaddMail
{
	...
	if($data->{'MAIL_TYPE'} =~ /_mail/) {
		...
		my $mailDir = "$mta->{'postfixConfig'}->{'MTA_VIRTUAL_MAIL_DIR'}/$data->{'DOMAIN_NAME'}/$data->{'MAIL_ACC'}";
		my $mailUidName =  $mta->{'postfixConfig'}->{'MTA_MAILBOX_UID_NAME'};
		my $mailGidName = $mta->{'postfixConfig'}->{'MTA_MAILBOX_GID_NAME'};

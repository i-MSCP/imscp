...
class SystemInfo {
	...
	private function _getCPUInfo() {
		$cpu = array("model" => tr("N/A"), "cpus" => tr("N/A"), 
				"cpuspeed" => tr("N/A"), "cache" => tr("N/A"), 
				"bogomips" => tr("N/A"));
		...
		if (PHP_OS == "FreeBSD" || PHP_OS == "OpenBSD" || PHP_OS == "NetBSD") {
			...
			if (empty($this->error)) {
				...
				$sparclist = array('SUNW,UltraSPARC@0,0', 'SUNW,UltraSPARC-II@0,0', 
					'SUNW,UltraSPARC@1c,0', 'SUNW,UltraSPARC-IIi@1c,0', 
					...
					foreach ($cpu_info as $ci) {
						...
						switch($line[0]) {
							case "Processor":
								...
								$cpu["model"] = $line[1];
								break;
							case "BogoMIPS": // Wrong description for CPU speed; no bogoMIPS available
								$cpu["cpuspeed"] = $line[1];
								break;
							case "I size":
								$cpu["cache"] = $line[1];
	...
	private function _getSwapInfo() {
		...
		if (PHP_OS == "FreeBSD" || PHP_OS == "OpenBSD" || PHP_OS == "NetBSD") {
			...
			$proc = proc_open(Config::get("CMD_SWAPCTL") . $args, 
	...
	protected function read($filename) {
		...
		if ($result === false) {
			$this->error = sprintf(tr("File %s does not exists or cannot be reached!"), $filename);
			return false;
		} else {
			$this->error = "";
			return $result;

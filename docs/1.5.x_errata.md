...
Support for PHP 7.2 and PHP 7.3 has been added. Default selected PHP
version for customers is now PHP 7.3.

Note that all available PHP versions are now always installed. This make the
`PhpSwitcher` plugin able to automatically make them available for customers
instead of relying on self-compiled PHP versions.

### Plugins

i-MSCP plugins (frontEnd) are now executed through the
[Slim](http://www.slimframework.com) micro-framework. This shouldn't break
compatibility with older plugin versions as execution of those is simply
wrapped in a specific callback.

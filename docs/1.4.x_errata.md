...
Due to a bug found in previous i-MSCP versions, people that use
Debian 9/Stretch or Ubuntu 16.04/Xenial must execute the following command
manually:
...
for RSYSLOGD(8) which has been removed by mistake, leading to several log files
not being rotated.

Once done, it could be also necessary to trigger log rotation manually as
follows:

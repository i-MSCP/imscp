...
Starting with this new version, usage of SQL root user is prohibited. Instead, a dedicated SQL user, for i-MSCP will be
created. This change intends to solve issues with SQL servers that are pre-configured to use new authentication plugins
such as unix_socket authentication plugin. With such a configuration, the password for the SQL root user is no set
while server package installation. Instead, the authentication is done through a unix socket. The plugin does a mapping
between the local unix user and the SQL user.

Note that during installation or reconfiguration, installer will still ask you for the SQL root user info when needed.
This user will be only used to create and grant privileges to i-MSCP SQL users. This also means that i-MSCP will never
store any data related to the SQL root user, nor change any of its properties.

Be also aware that in latest version of PhpMyAdmin package, usage of SQL root user is prohibited for security reasons.
If want connect to PhpMyAdmin to work on all databases, you should now use the i-MSCP master SQL user.
...
Starting with this new version, permissions on Customer's files are no longer set recursively by default. To enable
recursion, you must now pass the `--fix-permissions` option to the installer (or any script supporting it). Doing this
allows to mitigate long running processes when a customer has thoussant of files in its Web folders or mail directory.
...
The `--fix-permissions` option is supported by the following scripts:
 ...
 - set-engine-permissions (Script that set engine permissions)

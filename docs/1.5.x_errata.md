...
Following a security policy hardening, authentication data that belong to the
i-MSCP master SQL user no longer stored into the `/etc/mysql/imscp.cnf`
configuration file.

In previous versions, the root user was able to connect to MySQL server through

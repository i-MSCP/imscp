...
function &reseller_getData($domainId, $forUpdate = false)
{
    ...
    // Load reseller and client PHP permissions if needed
    if ($data['reseller_php_ini_system'] == 'yes') {
        $phpini = iMSCP_PHPini::getInstance();
        $phpini->loadResellerPermissions($data['reseller_id']);
...
function reseller_checkAndUpdateData($domainId)
{
    ...
    try {
        ...
        $phpEditorOld = array_merge($phpEditor->getDomainIni(), $phpEditor->getClientPermission());

        if (isset($_POST['php_ini_system']) && $phpEditor->resellerHasPermission('phpiniSystem')) {
            $phpEditor->setClientPermission('phpiniSystem', clean_input($_POST['php_ini_system']));

            if ($phpEditor->clientHasPermission('phpiniSystem')) {
                if (isset($_POST['phpini_perm_allow_url_fopen'])) {
                    $phpEditor->setClientPermission('phpiniAllowUrlFopen', clean_input($_POST['phpini_perm_allow_url_fopen']));
                }

                if (isset($_POST['phpini_perm_display_errors'])) {
                    $phpEditor->setClientPermission('phpiniDisplayErrors', clean_input($_POST['phpini_perm_display_errors']));
                }

                if (isset($_POST['phpini_perm_disable_functions'])) {
                    $phpEditor->setClientPermission('phpiniDisableFunctions', clean_input($_POST['phpini_perm_disable_functions']));

                    //if (in_array($phpEditor->getClientPermission('phpiniDisableFunctions'), array('exec', 'no'))) {
                    //    $phpEditor->setDomainIni('phpiniDisableFunctions', $phpEditor->getDefaultIni('phpiniDisableFunctions'));
                    //}
                }

                if (isset($_POST['phpini_perm_mail_function'])) {
                    $phpEditor->setClientPermission('phpiniMailFunction', clean_input($_POST['phpini_perm_mail_function']));
                }

                if (isset($_POST['post_max_size'])) {
                    $phpEditor->setDomainIni('phpiniPostMaxSize', clean_input($_POST['post_max_size']));
                }

                if (isset($_POST['upload_max_filezize'])) {
                    $phpEditor->setDomainIni('phpiniUploadMaxFileSize', clean_input($_POST['upload_max_filezize']));
                }

                if (isset($_POST['max_execution_time'])) {
                    $phpEditor->setDomainIni('phpiniMaxExecutionTime', clean_input($_POST['max_execution_time']));
                }

                if (isset($_POST['max_input_time'])) {
                    $phpEditor->setDomainIni('phpiniMaxInputTime', clean_input($_POST['max_input_time']));
                }

                if (isset($_POST['memory_limit'])) {
                    $phpEditor->setDomainIni('phpiniMemoryLimit', clean_input($_POST['memory_limit']));
                }
            } else {
                $phpEditor->loadClientPermissions(); // Reset client permissions
                $phpEditor->loadDomainIni(); // Reset domain PHP configuration options
            }
        }

        // Needed to check if something changed (see below)
        $phpEditorNew = array_merge($phpEditor->getDomainIni(), $phpEditor->getClientPermission());
        ...
        if (empty($errFieldsStack) && !Zend_Session::namespaceIsset('pageMessages')) { // Update process begin here
            ...
            if ($phpEditorNew != $phpEditorOld) {
                updateCustomerPhpPerms($data['admin_id']);
            }
            ...
            exec_query(
                ...
                array(
                    ...
                    $phpEditor->getClientPermission('phpiniSystem'),
                    $phpEditor->getClientPermission('phpiniAllowUrlFopen'),
                    $phpEditor->getClientPermission('phpiniDisplayErrors'),
                    $phpEditor->getClientPermission('phpiniDisableFunctions'),

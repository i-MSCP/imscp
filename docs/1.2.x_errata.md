...
### Custom SPF DNS records

To fulfit specific requirements, the SPF DNS record type as been added to the list of allowed custom DNS records.
It is now possible to define custom SPF record using SPF and TXT DNS records. Note also that when a custom SPF DNS
record is detected (SPF or TXT), the default SPF DNS records set by i-MSCP are automatically removed.

Note: As per the RFC 7208, the SPF DNS record is deprecated. However it is still required in some contexts.

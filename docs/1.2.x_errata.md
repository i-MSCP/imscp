...
Be aware that the transitional **iMSCP::HooksManager** package, which was an alias of the **iMSCP::EventManager** since
i-MSCP version **1.1.14**, has been removed. Thus, if you're using that package name in one of your listeners, you must
change it to **iMSCP::EventManager**, which is  the real package name.

=head1 NAME
 ...
 Package::WebStatistics::AWStats::Installer - AWStats package installer
...
package Package::WebStatistics::AWStats::Installer;
...
=head1 DESCRIPTION
 ...
 See Package::WebStatistics::AWStats::AWStats for more information.
...
=item _init( )
 ...
 Return Package::WebStatistics::AWStats::Installer
...
sub _setupApache2
{
    ...
    $rs = $self->{'httpd'}->buildConfFile( "$::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Package/WebStatistics/AWStats/Config/01_awstats.conf" );
...
sub _disableDefaultConfig
{
    ...
    if ( -f "$::imscpConfig{'AWSTATS_CONFIG_DIR'}/awstats.conf" ) {
        $rs = iMSCP::File->new( filename => "$::imscpConfig{'AWSTATS_CONFIG_DIR'}/awstats.conf" )->moveFile(

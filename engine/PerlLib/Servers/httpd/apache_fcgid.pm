...
use iMSCP::File;
use iMSCP::Getopt;
use iMSCP::Mount qw / mount umount isBindMount addMountEntry removeMountEntry /;
use iMSCP::Net;
...
{
    Servers::httpd::apache_fcgid::installer->getInstance()->registerSetupListeners( $_[1] );
}
...
sub mountLogsFolder
{
    ...
    $rs ||= mount( $fields ) unless isBindMount( $fields->{'fs_file'} );
    $rs ||= mount( $fields );

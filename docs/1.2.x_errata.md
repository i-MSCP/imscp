...
VsFTPd doesn't support non-ascii characters in usernames. Therefore, to be compatible with VsFTPD and
ProFTPD, the internationalized domain names (IDN) that are part of FTP usernames will be converted to IDNA form.
This means that only ASCII usernames will be accepted.

You must not forgot to warn your customers about this change.

### i-MSCP master SQL user

Starting with this new version, usage of SQL root user is prohibited. Instead, a dedicated SQL user, for i-MSCP
will be created. This change intends to solve issues with SQL servers that are pre-configured to use new
authentication plugins such as unix_socket authentication plugin. With such a configuration, the password for
the SQL root user is no set while server package installation. Instead, the authentication is done through a unix
socket. The plugin does a mapping between the local unix user and the SQL user.

Note that during installation or reconfiguration, installer will still ask you for the SQL root user info when
needed. This user will be only used to create and grant privileges to i-MSCP SQL users. This also means that
i-MSCP will never store any data related to the SQL root user, nor change any of its properties.
...
This new parameters allows the administrator to disable mount of customers httpd log directories. To disable
them, you can process as follow:
...
Be aware that when mount of httpd log directories is disabled, the 'logs' directory located in the home directory
of customers is also removed.

### Services

#### imscp_mountall service

This service mounts i-MSCP file systems when the server is rebooted. This service reads entries in a fstab-like
file located at /etc/imscp/mounts/mounts.conf. Unlike the entries that are added in the system fstab file, the
entries added in this file are processed in sequential order.
...
resource records. It is now possible to setup custom SPF records using SPF and TXT DNS resource records. However
you must be aware that when a custom SPF DNS resource record is detected (SPF or TXT), the default SPF DNS
resource records set by i-MSCP are automatically removed.

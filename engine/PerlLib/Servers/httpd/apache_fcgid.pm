...
sub preinstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdPreinstall', 'apache_fcgid');
	$self->stop();
...
sub install
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdInstall', 'apache_fcgid');

	require Servers::httpd::apache_fcgid::installer;
	my $rs = Servers::httpd::apache_fcgid::installer->getInstance()->install();
	return $rs if $rs;
...
sub postinstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdPostInstall', 'apache_fcgid');
	iMSCP::Service->getInstance()->enable($self->{'config'}->{'HTTPD_SNAME'});
	$self->{'eventManager'}->register(
		'beforeSetupRestartServices', sub { push @{$_[0]}, [ sub { $self->start(); }, 'Apache2/Fcgid httpd server' ]; 0; }
	);
...
sub uninstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdUninstall', 'apache_fcgid');

	require Servers::httpd::apache_fcgid::uninstaller;
	my $rs = Servers::httpd::apache_fcgid::uninstaller->getInstance()->uninstall();
	return $rs if $rs;

	$self->{'eventManager'}->trigger('afterHttpdUninstall', 'apache_fcgid');
	$self->restart();
}
...
sub setEnginePermissions
{
	...
	require Servers::httpd::apache_fcgid::installer;
	my $rs = Servers::httpd::apache_fcgid::installer->getInstance()->setEnginePermissions();
	return $rs if $rs;
...
sub deleteDmn
{
	...
	iMSCP::Dir->new( dirname => "$self->{'config'}->{'PHP_STARTER_DIR'}/$data->{'DOMAIN_NAME'}" )->remove();
...
sub buildConfFile
{
	...
	$self->{'eventManager'}->trigger('onLoadTemplate', 'apache_fcgid', $filename, \my $cfgTpl, $data);
...
sub _buildPHPConfig
{
	...
	if($data->{'FORWARD'} eq 'no' && $data->{'PHP_SUPPORT'} eq 'yes') {
		...
		iMSCP::Dir->new( dirname => $fcgiDir )->remove();
		...
		my $rs = $self->buildConfFile("$main::imscpConfig{'CONF_DIR'}/fcgi/parts/php-fcgi-starter.tpl", $data, {
		...
		$rs = $self->buildConfFile("$main::imscpConfig{'CONF_DIR'}/fcgi/parts/php$phpVersion/php.ini", $data, {

...
then
	for a in `cat /usr/local/etc/ispcp/ispcp.conf  | grep -E '(^ROOT_DIR|^APACHE_WWW_DIR|^PHP_STARTER_DIR)' | sed -e 's/ //g'`; do
		export $a
	done
else
	for a in `cat /etc/ispcp/ispcp.conf | grep -E '(^ROOT_DIR|^APACHE_WWW_DIR|^PHP_STARTER_DIR)' | sed -e 's/ //g'`; do
		export $a
	done
fi
...
for wdir in ${WEBDIRS}; do
	...
	if [ ! -f "${fcgidir}/php4/php.ini" ] && [ ! -f "${fcgidir}/php5/php.ini" ]; then
		continue;
	fi
...
tmpdir="${ROOT_DIR}/gui/phptmp"
fcgidir=${PHP_STARTER_DIR}/master
if [ -f "${fcgidir}/php4/php.ini" ] || [ -f "${fcgidir}/php5/php.ini" ];then
	cur=0
	max=1440
	for ini in "${fcgidir}/php4/php.ini ${fcgidir}/php5/php.ini"; do

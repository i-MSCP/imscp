...
class iMSCP_Config_Handler_Db extends iMSCP_Config_Handler implements iterator
{
     ...
     *
     * <b>Note:</b> For performance reason, the PDOStatement instance is created only
     * once at the first execution of the {@link _insert()} method.
     *
     * @var PDOStatement
     */
     ...
     *
     * <b>Note:</b> For performances reasons, the PDOStatement instance is created
     ...
     *
     * This variable is bound to the PDOStatement instances that are used by both
     ...
     * <b>Parameters:</b>
     *
     * The constructor accepts one or more parameters passed in a array where each
     * key represent a parameter name.
     *
     ...
     *
     * <b>Note:</b> For performances reasons, queries for updates are only done if
     * old and new value of a parameter are not the same.
     *
     * @param string $key Configuration parameter key name
     * @param mixed $value Configuration parameter value
     * @return void
     ...
     * value is NULL. To test existence of a configuration parameter, you should use
     ...
     *
     * This method will remove all the current loaded parameters and reload it from
     * the database.
     *
     * @return void
     */
     ...
     *
     * This method returns the count of queries that were executed since the last
     * call of {@link reset_queries_counter()} method.
     *
     * @throws iMSCP_Exception
    ...
    protected function _insert()
    {
        ...
        if (!$this->_insertStmt->execute(array(
                                              ':index' => $this->_key,
                                              ':value' => $this->_value))
        ) {
    ...
    protected function _update()
    {
        ...
        if (!$this->_updateStmt->execute(array(
                                              ':index' => $this->_key,
                                              ':value' => $this->_value))
        ) {
            throw new iMSCP_Exception_Database(
                "Unable to update entry `{$this->_key}` in config table.");
        } else {
    ...
    protected function _delete()
    {
        ...
        if (!$this->_deleteStmt->execute(array(':index' => $this->_key))) {
            throw new iMSCP_Exception_Database(
                'Unable to delete entry in config table.');
        } else {
     ...
     *
     * <b>Note:</b> This is the first method called when starting a foreach loop. It
     * will not be executed after foreach loops.
     *
     * @return void
     */

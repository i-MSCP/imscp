...
reconfiguration.
- You can disableAPCu by enabling the debugging mode in the
`/etc/imscp/imscp.conf` file.

### PHP version

The i-MSCP FrontEnd now makes use of the best supported PHP version, regadless
of the selected PHP alternative. In other words, even when the PhpSwitcher
plugin is not used, the PHP version used by the i-MSCP frontEnd can be other
than the PHP version used by customers.

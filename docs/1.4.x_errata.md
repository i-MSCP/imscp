...
### Alternative URLs for customer domains

Alternative URL feature can now be disabled. In order, to disable/enable that
 feature, you must:
 
- Edit the /etc/imscp/imscp.conf file and change the value of the
  `CLIENT_DOMAIN_ALT_URLS` configuration parameter either to `1` to enable the
  feature of `0` to disable it
- Execute the imscp-reconfigure script as follow:

```
perl /var/www/imscp/engine/setup/imscp-reconfigure -dasnv
```

There is no better way to disable/enable that feature because the DNS and
Apache2 configuration files must be regenerated.
...
i-MSCP Apache2 template files were updated. If you use your own template files,
don't forget to sync them.

### DNS server (Bind9)

#### Templates

i-MSCP Bind9 template files were updated. If you use your own template files,
don't forget to sync them.

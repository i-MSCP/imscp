...
BACKEND
	Added: beforeMountHomedir and afterMountHomedir
	Added: beforeMountLogsFolder, afterMountLogsFolder, beforeUnmountLogsFolder and afterUmountMountLogsFolder events
	Added: imscp-mountall-mngr script - Mount homedirs and httpd logs folders
	...
	Added: onBoot event - Triggered once the backend library has been initialized. Receive boot mode as parameter
...
CONFIGS
	Added: imscp_mountall init script - Mount homedirs and httpd logs folders on reboot
	Added: imscp_mountall upstart job - Mount homedirs and httpd logs folders on reboot
	...
	Fixed: ftp_ssl.log log file is not rotated (proftpd logrotate configuration file)
...
INSTALLER
	Changed: Minimum PHP required PHP version is now 5.3.7
...
YOUTRACK
	...
	Fixed: #IP-1417 Bug - When changing hostname using the panel_hostname reconfiguration option, The SSL dialog should be triggered

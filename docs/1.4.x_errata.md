...
domain, and create new mailboxes for that domain. From Postfix point of view, the server hostname, which is added in the
[mydestination](https://postfix.traduc.org/index.php/postconf.5.html#mydestination) parameter, is a canonical domain for
...
`catch-all` accounts for that domain.
...
This change should help mitigate upgrade errors caused by plugins that are not compatible with newst i-MSCP versions.

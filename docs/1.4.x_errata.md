...
Due to a bug found in previous i-MSCP version, people that use Debian 9/Stretch
or Ubuntu 16.04/Xenial must execute the following command manually:

```
apt-get -o DPkg::Options::='--force-confmiss' --reinstall install rsyslog
```

This will force reinstallation of the missing LOGROTATE(8) configuration file
for RSYSLOGD(8) which was removed by mistake in previous i-MSCP versions,
leading to several log files not being rotated.

It could be also necessary to trigger log rotation manually as follows:

```
logrotate --verbose --force /etc/logrotate.d/rsyslog
```

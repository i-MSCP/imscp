...
class PHPMailer
{
    ...
    /**
     *  Read only - return a formated mail to the client
     *  @var string
     */
    var $FormatedMail          = "";
    ...
    */
    ...
    function CreateHeader() {
        ...
        $result .= $this->HeaderLine("X-Mailer", "UebiMiau [PHPMailer version " . $this->Version . "]");
        ...
        switch($this->message_type)
        {
            case "plain":
                ...
                $result .= sprintf("Content-Type: %s; ".$this->LE."\tcharset=\"%s\"",
    ...
    function Received() {
        ...
        $result = sprintf("Received: from %s %s\tby %s " .
                          "with %s (UebiMiau);%s\t%s%s", $remote, $this->LE,

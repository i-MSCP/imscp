#!/usr/bin/perl
#
# imscp-eol - Convert all EOL of i-MSCP files to Unix format and adding missing EOL
...
my $discard_files = '^.*\.(gif|ico|idea|jpg|mo|png|po|project|ser|sfd|svn|swf|ttf|wav|z)|filemanager|pma|webmail$';
...
if ($verbose){
	...
	Note: The directories/files corresponding to the following pattern:
		...
		/^.*\.(gif|ico|idea|jpg|mo|png|po|project|svn|ser|sfd|swf|ttf|wav|z)|filemanager|pma|webmail\$/

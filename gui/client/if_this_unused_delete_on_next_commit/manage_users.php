...
$tpl = new pTemplate();
$tpl -> define_dynamic('page', $cfg['CLIENT_TEMPLATE_PATH'].'/manage_users.tpl');
  ...
  if (preg_match('/_mail$/', $mail_type) == 1) {
    if ($mail_status === $cfg['ITEM_OK_STATUS']) {
      if ($mail_auto_respond === '_no_') {
        $tpl -> assign(array('AUTO_RESPOND_ACTION' => tr('Enable'),
                             'AUTO_RESPOND_ACTION_SCRIPT' => "enable_mail_arsp.php?id=$mail_id"));
      } else {
        $tpl -> assign(array('AUTO_RESPOND_ACTION' => tr('Disable'),
...
SQL_QUERY;
  ...
  if ($rs -> RecordCount() == 0) {
    ...
    while (!$rs -> EOF) {
      ...
      $tpl -> assign(array('MAIL_ACC' => $mail_acc."@".$show_dmn_name,
...
SQL_QUERY;
  ...
  if ($rs -> RecordCount() == 0) {
    ...
    while (!$rs -> EOF) {
      ...
      $show_dmn_name = decode_idna($dmn_name);
      $tpl -> assign(array('MAIL_ACC' => $mail_acc."@".$show_sub_name.".".$show_dmn_name,
                           'MAIL_TYPE' => user_trans_mail_type($rs -> fields['mail_type']),
...
SQL_QUERY;
  ...
  if ($rs -> RecordCount() == 0) {
    ...
    while (!$rs -> EOF) {
      ...
      $tpl -> assign(array('MAIL_ACC' => $mail_acc."@".$show_als_name,
                           'MAIL_TYPE' => user_trans_mail_type($rs -> fields['mail_type']),
                           'MAIL_STATUS' => user_trans_item_status($rs -> fields['status']),
                           'MAIL_ACTION' => $mail_action,
                           'MAIL_ACTION_SCRIPT' => $mail_action_script,
...
function gen_catchall_item(&$tpl, $action, $dmn_id, $dmn_name, $mail_id, $mail_acc, $mail_status, $ca_type)
{
  $show_dmn_name = decode_idna($dmn_name);
  if ($action === 'create') {
    $tpl -> assign(array('CATCHALL_DOMAIN' => $show_dmn_name,
...
function gen_page_catchall_list(&$tpl, &$sql, $dmn_id, $dmn_name, $dmn_mails, $als_mails)
{
  ...
  if ($total_mails == 0) {
    $tpl -> assign(array('CATCHALL_MSG' => tr('Catch all mail account can not be created!'),
                         'CATCHALL_ITEM' => ''));
...
SQL_QUERY;
      ...
      $rs_als = exec_query($sql, $query, array($dmn_id, $als_id));
...
SQL_QUERY;
  ...
  if ($rs -> RecordCount() == 0) {
      $tpl -> assign(array('FTP_MSG' => tr('FTP list is empty!'),
                           'FTP_ITEM' => '',
                           'FTPS_TOTAL' => ''));
      ...
      for ($i = 0; $i < count($ftp_accs); $i++) {
        ...
        $tpl -> assign(array('FTP_ACCOUNT' => $ftp_accs_encode[$i],
                             'UID' => $ftp_accs[$i]));
...
function gen_page_lists(&$tpl, &$sql, $user_id)
{
  ...
  if ($total_mails > 0) {
    ...
    $tpl -> assign(array('MAIL_MSG' => tr('Mail accounts list is empty!'),
                         'MAIL_ITEM' => '',
                         'MAILS_TOTAL' => ''));
...
$tpl -> assign(array('TR_CLIENT_MANAGE_USERS_PAGE_TITLE' => tr('VHCS - Client/Manage Users'),
...
$tpl -> assign(array('TR_MANAGE_USERS' => tr('Manage users'),

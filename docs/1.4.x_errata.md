...
From now on, if an address IP that is assigned to a customer is not known by the system (not configured), both the
domain and alias modules will automatically fallback to the primary server IP address.

Doing this should help mitigating upgrade and migration problems when an IP address is not longer available on the
system.

Note that the modules won't update the enries in the database. The fallback is made at backend level. The administrator
will still needs have to solve the situation through the frontEnd by following these steps:

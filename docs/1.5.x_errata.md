...
This is a maintenance release for the 1.5.3 version with few new features.
...
their domain(s) and/or subdomain(s). This feature is most suitable for software
...
records more easily when those are not longer overridden.

#### Wildcard DNS records

Customers can now add wildcard DNS resource record entries through the custom
DNS interface.
...
processing on the first failure. This was not a good behavior as not all
entities belong to the same aggregate root (RA).
...
First, you need change the default port in the /etc/imscp.conf file to make the
...
then, you need override the init script as follows:
...
The FTP, HTTP, IMAP/POP and SMTP service are no longer stopped while an i-MSCP

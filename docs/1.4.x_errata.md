...
Apache2 (2.4.10) is now installed backports repository. This allow us to not longer depends on the Apache2 fastcgi
module when the PHP SAPI is FPM.

### Distribution package files

Distribution package files were updated. If you're using the 10_packages_override.pl i-MSCP listener file to override
your distirbution package file, don't forget to sync it.
...
from each mailbox maildirsize file and cached for 5 minutes. Note that if the file doesn't exists yet, no quota
information will be showed.

In case of Courier IMAP/POP server, that file will be automatically created on mailbox account creation/update while in
case of Dovecot IMAP/POP server usage, that file will be created on first IMAP login. Note that if there is not quota
set for a mailbox, no quota information will be showed.

			...
			if($rs->fields['created_by'] == '' ||
				...
				$tpl->assign(
					array(
						'ADMIN_DELETE_SHOW'     => '',
						'TR_DELETE'             => tr('Delete'),
						'URL_DELETE_ADMIN'      => 'user_delete.php?delete_id=' . $rs->fields['admin_id'] .
							'&amp;delete_username=' . $rs->fields['admin_name'],
			...
			if($rs->fields['created_by'] == '') {
				...
				$tpl->assign(
					array(
						'RSL_DELETE_SHOW'           => '',
						'TR_DELETE'                 => tr('Delete'),
						'URL_DELETE_RSL'            => 'user_delete.php?delete_id=' . $rs->fields['admin_id'] .
							'&amp;delete_username=' . $rs->fields['admin_name'],
...
function gen_user_list($tpl, $sql) {
	...
	if(isset($_SESSION['search_for'])) {
		gen_admin_domain_query(
			$search_query, $count_query, $start_index, $rows_per_page, $_SESSION['search_for'],
...
function generate_reseller_props($reseller_id) {
	...
	return array(
		$rs->fields['current_dmn_cnt'], $rs->fields['max_dmn_cnt'], $rs->fields['current_sub_cnt'],
		$rs->fields['max_sub_cnt'], $rs->fields['current_als_cnt'], $rs->fields['max_als_cnt'],
		$rs->fields['current_mail_cnt'], $rs->fields['max_mail_cnt'], $rs->fields['current_ftp_cnt'],
		$rs->fields['max_ftp_cnt'], $rs->fields['current_sql_db_cnt'], $rs->fields['max_sql_db_cnt'],
		$rs->fields['current_sql_user_cnt'], $rs->fields['max_sql_user_cnt'], $rs->fields['current_traff_amnt'],
		$rs->fields['max_traff_amnt'], $rs->fields['current_disk_amnt'], $rs->fields['max_disk_amnt']
	);
}
...
function generate_user_traffic($user_id) {
	...
	if($rs->rowCount() == 0 || $rs->rowCount() > 1) {
		write_log(
			'TRAFFIC WARNING: ' . $rs->fields['domain_name'] . ' manages incorrect number of domains: ' . $rs->rowCount()
		);
		...
		return array(
			$domain_name, $domain_id, $rs1->fields['web'], $rs1->fields['ftp'], $rs1->fields['smtp'], $rs1->fields['pop'],
			$rs1->fields['total'], $domain_disk_usage, $domain_traff_limit, $domain_disk_limit
		);
	}
}
...
function get_logo($user_id) {
	...
	$query = "SELECT `admin_id`, `created_by`, `admin_type` FROM `admin` WHERE `admin_id` = ?;";
...
function update_reseller_props($reseller_id, $props) {
	...
	$res = exec_query(
		$sql, $query,
		array(
			$dmn_current, $dmn_max, $sub_current, $sub_max, $als_current, $als_max, $mail_current, $mail_max,
			$ftp_current, $ftp_max, $sql_db_current, $sql_db_max, $sql_user_current, $sql_user_max, $traff_current,
			$traff_max, $disk_current, $disk_max, $reseller_id
		)
	);
 ...
 * @return void
 */
function gen_admin_domain_query(&$search_query, &$count_query, $start_index, $rows_per_page, $search_for, $search_common,
	$search_status) {
	...
	if($search_for == 'n/a' && $search_common == 'n/a' && $search_status == 'n/a') {
		...
		$search_query = "
			SELECT * FROM `domain` $add_query ORDER BY `domain_name` ASC LIMIT $start_index, $rows_per_page;
		";
	} else if($search_for != '') {
			...
			if ($sw_depot) {
				$updatequery = "
					UPDATE
						`web_software_inst`
					SET
						`software_id` = ?, `software_master_id` = ?, `software_res_del` = 0
					WHERE
						`software_name` = ?
					AND
						`software_version` = ?
				...
				exec_query(
					$sql, $updatequery,
					array(
						$software_id, $software_master_id, $software_name, $software_version, $software_language,
				...
				exec_query(
					$sql, $updatequery,
					array(
						$software_id, $software_name, $software_version, $software_language, $res->fields['domain_id']
					)
				);
			}
			$res->MoveNext();
...
function update_existing_client_installations_sw_depot($software_id, $software_master_id, $reseller_id) {
	...
	if ($res->RecordCount() > 0) {
		while(!$res->EOF) {
			...
			exec_query($sql, $updatequery, array($software_id, $software_master_id, $res->fields['domain_id']));

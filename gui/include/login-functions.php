...
function session_exists($sessionId) {
	...
	$stmt = exec_query($sql, $query, array($sessionId, $ip));
...
function is_userdomain_expired($userName) {
	...
	$stmt = exec_query($sql, $query, $userData['admin_id']);
...
function is_userdomain_ok($userName) {
	...
	if ($userData['admin_type'] != 'user') return true;
...
function is_ipaddr_blocked($ipAddress = null, $type = 'bruteforce', $autoDeny = false) {
	...
	$stmt = exec_query($sql, $query, array($ipAddress, $max));
...
function check_ipaddr($ipAddress = null, $type = 'bruteforce') {
	...
	if ($stmt->recordCount() == 0) {
		...
		exec_query(
			$sql, $query, array($sessionId, $ipAddress, (int) ($type == 'bruteforce'), (int) ($type == 'captcha'))
		);
	...
	if (!is_null($fileName)) {
		...
		if($userType != $level) {
			if($userType != 'admin' &&
				...
				write_log(
					'Warning! user |' . $userLoggued . '| requested |' . tohtml($_SERVER['REQUEST_URI']) .

...
function get_application_installer_conf()
{
    ...
    if (!$stmt->rowCount()) {
        throw new Exception(
            'Unable to retrieve software installer options in database'
        );
    }
    ...
    return [
        $row['use_webdepot'], $row['webdepot_xml_url'],
        $row['webdepot_last_update']
    ];
}

/**
 * Check whether the package is still installed this system
 *
...
function check_package_is_installed(
    $packageInstallType, $packageName, $packageVersion, $packageLanguage,
    $userId
)
{
    $stmt = exec_query(
        'SELECT admin_type FROM admin WHERE admin_id = ?', [$userId]
    );

    if (!$stmt->rowCount()) {
        throw new Exception('Unable to found the given user in database');
    }
    ...
    $softwareCountDepot = $stmt->rowCount();

    if ($softwareCount || $softwareCountDepot) {
        if ($softwareCount) {
...
/**
 * Get all software packages from database since last update from the
 * websoftware depot
 *
...
function get_webdepot_software_list($tpl, $userId)
{
    $stmt = execute_query(
        '
            SELECT *
            FROM web_software_depot
            ORDER BY package_install_type ASC, package_title ASC
        '
    );
    ...
    if ($rowCount) {
        while ($row = $stmt->fetchRow()) {
            $tpl->assign([
                ...
                'TR_PACKAGE_TOOLTIP'      => tohtml(
                    $row['package_description'], 'htmlAttr'
                ),
                'TR_PACKAGE_INSTALL_TYPE' => tohtml(
                    $row['package_install_type']
                ),
                ...
                'TR_PACKAGE_VENDOR_HP'    => $row['package_vendor_hp'] === ''
                    ? tr('N/A') : '<a href="' . $row['package_vendor_hp']
                    . '" target="_blank">' . tr('Vendor hompage') . '</a>'
            ]);
            ...
            if ($isInstalled) {
                ...
                $tpl->assign([
                    ...
                    'TR_MESSAGE_INSTALL' => tr(
                        'Are you sure you want to install this package from the Web software repository?'
                    )
                ]);
        ...
        $tpl->assign([
            'NO_WEBDEPOTSOFTWARE_AVAILABLE' => tr(
                'No software in Web repository found!'
            ),
 ...
 */
function update_webdepot_software_list(
    $repositoryIndexFile, $webRepositoryLastUpdate
)
{
    ...
    if (utf8_decode($webRepositoryIndexFile->LAST_UPDATE->DATE)
        != $webRepositoryLastUpdate
    ) {
        ...
        foreach ($webRepositoryIndexFile->PACKAGE as $package) {
            /** @noinspection PhpUndefinedFieldInspection */
            if (!empty($package->INSTALL_TYPE)
                && !empty($package->TITLE)
                && !empty($package->VERSION)
                && !empty($package->LANGUAGE)
                && !empty($package->TYPE)
                && !empty($package->DESCRIPTION)
                && !empty($package->VENDOR_HP)
                && !empty($package->DOWNLOAD_LINK)
                ...
                exec_query(
                    $query,
                    [
                        clean_input($package->INSTALL_TYPE),
                        clean_input($package->TITLE),
                        clean_input($package->VERSION),
                        clean_input($package->LANGUAGE),
                        clean_input($package->TYPE),
                        clean_input($package->DESCRIPTION),
                        encode_idna(
                            strtolower(clean_input($package->VENDOR_HP))
                        ),
                        encode_idna(
                            strtolower(clean_input($package->DOWNLOAD_LINK))
                        ),
                        encode_idna(
                            strtolower(clean_input($package->SIGNATURE_LINK))
                        )
                    ]);
            } else {
        ...
        if (!$badSoftwarePackageDefinition) {
            ...
            set_page_message(
                tr('Web software repository index been successfully updated.'),
                'success'
            );
        } else {
            set_page_message(
                tr(
                    'Update of Web software repository index has been aborted. Missing or empty fields.'
                ),
                'error'
            );
        }
        ...
        set_page_message
        (tr('Web software repository index is already up to date.'),
            'info'
        );
    }
}

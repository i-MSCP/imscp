 ...
 * @version     0.0.1
 */
class iMSCP_Debug_Bar_Plugin_Timer extends iMSCP_Debug_Bar_Plugin implements
    iMSCP_Events_Listeners_Interface
{
     ...
     *
     * The content for tab is the time elapsed since the script begin (Including
     * bootstrap and initialization steps)
     *
     * @return string
     */
    public function getTab()
    {
        return round($this->_times['scriptEnd'], 2) . ' ms';
    }
    ...
    public function getPanel()
    {
        ...
        $xhtml .= 'Current script (Bootstrap and initialization steps excluded): ' .
                  round(($this->_times['scriptEnd'] - $this->_times['scriptStart']), 2) .
                  ' ms<br />';

        if (isset($this->_timer['custom']) && count($this->_timer['custom'])) {
            foreach ($this->_timer['custom'] as $name => $time) {
    ...
    {
        if (isset($this->_timer['custom'][$name])) {
            $this->_timer['custom'][$name] =
                (microtime(true) - $_SERVER['REQUEST_TIME']) * 1000 - $this->_timer['custom'][$name];
        } else {
            $this->_timer['custom'][$name] =
                (microtime(true) - $_SERVER['REQUEST_TIME']) * 1000;
        }
    ...
    {
        $this->_times['scriptEnd'] = (microtime(true) - $_SERVER['REQUEST_TIME']) * 1000;
    }

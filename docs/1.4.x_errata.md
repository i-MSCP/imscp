...
First of all, if you're updating from a older i-MSCP Serie, you MUST read the archived errata files that you can find in
the ./docs directory of the i-MSCP archive.

## Version 1.4.4

### Distributions

Support for Devuan Jessie 1.0 RC2 has been added. See the [Devuan Project Page](https://devuan.org/) for further details.
...
Zone files are now stored in binary format for for rapid loading by named.
...
It is now possible to pass an object as event listener. For instance the following script:
...
Note that this option is more suitable for cloud computing services such as Scaleway.
...
Doing this should help mitigating upgrade or migration problems when an IP address is not longer available on the system.

Note that the modules won't update the enries in the database. The fallback is made at backend level. After upgrade, the
administrator will still needs to solve the situation through the frontEnd by following these steps:

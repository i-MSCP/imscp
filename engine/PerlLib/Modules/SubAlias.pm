...
sub _getHttpdData
{
	...
	unless($self->{'httpd'}) {
		...
		my $hstsMaxAge = $certData->{$self->{'subdomain_alias_id'}}->{'hsts_max_age'};
		my $hstsIncludeSubDomains = $certData->{$self->{'subdomain_alias_id'}}->{'hsts_includeSubDomains'} eq 'on' ? '; includeSubDomains' : '';

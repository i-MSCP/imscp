...
Starting from this new version, the installation program will no longer display
all the dialogs during a new installation. Only those for which no default
value can be provided will be displayed. This change was made to make life
easier for beginners.

You can still force all dialogs to appear by running the installer with the
`--reconfigure` command line option, even in the context of a new installation.

If you want known which are the default values, you can have a look at the
[preseeding](./preseed.pl) template file.

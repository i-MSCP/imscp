...
while (1 == 1)
{
    ...
    if ($DNSLookup) {
        ...
        elsif ($linerecord{$logfilechosen} =~ /([0-9A-F]*:)/i) {
            $ip = 6;
            $Host = $1;
        }                        # IPv6
        if ($ip) {
            ...
            if ($MyDNSTable{$Host}) {
                ...
                if (!$threadarray{$Host} && !$TmpDNSLookup{$Host}) {
                    ...
                    if ($Debug) { debug("  Reverse DNS lookup already queued or done for $Host: $TmpDNSLookup{$Host}",
                        4); }
                }
            }
            else {
    ...
    if ($Debug) { debug("Add record $NbOfLinesParsed in record queue (with host to resolve = ".($Host ? $Host : '*').")"
        , 4); }

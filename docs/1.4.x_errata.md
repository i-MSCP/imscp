...
There is now only one log file per module. In other word, event logging is now made on a per module basis. Before, event
logging was made on a per item basis, possibly leading to hundred files stored in the /var/log/imscp directory.

There is no reasons to kept logs for all processed items. Generally speaking, administrator only has interest to log of
last processed item, mostly when there was an error raised.

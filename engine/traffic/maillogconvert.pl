...
sub CleanHost
{
    ...
    if ( /^unknown\s*\[/ ) {
        $_ =~ /\[(.*)\]/;
        $_ = $1;
    }    # If unknown [ip], we keep ip else { $_ =~ s/\s*\[.*$//; }
...
sub OutputRecord
{
    ...
    # Clean to
    if ( $mode eq 'vadmin' ) { $to = &CleanVadminUser( $to ); } else { $to = &CleanEmail( $to ); }
    $to ||= '<>';
...
foreach my $key ( sort keys %param ) {
    ...
    if ( $param{$key} =~ /^(standard|vadmin)$/i ) {
        $mode = $1;
        shift;
        next;
    }
...
while ( <> ) {
    ...
    # elsif ( /: reject/ ) {
        ...
        if ( !$mon ) { ( $mon, $day, $time, $id, $code,
            $from ) = m/(\w+)\s+(\d+)\s+(\d+:\d+:\d+)\s+[\w\-\.\@]+\s+(?:postfix\/(?:local|lmtp|smtpd|smtp|virtual|pipe))\[\d+\]:\s+(.*?):\s+(.*)\s+from=([^\s,]*)/; }
        ...
        if ( $mailid ) {
            ...
            if ( $code =~ /\s+(\d\d\d)\s+/ ) { $mail{$mailid}->{'code'} = $1; } else { $mail{$mailid}->{'code'} = 999; }    # Unknown error
    ...
    # elsif ( /, reject/ ) {
        ...
        if ( !$mon ) { ( $mon, $day, $time, $id, $ruleset,
            $code ) = m/(\w+)\s+(\d+)\s+(\d+:\d+:\d+)\s+[\w\-\.\@]+\s+(?:sendmail|sm-mta)\[\d+\]:\s+(.*?):\s\w+:\s(\w+),\s+(reject=.*)/; }
        ...
        if ( $mailid ) {
            ...
            if ( $code =~ /=(\d\d\d)\s+/ ) { $mail{$mailid}->{'code'} = $1; } else { $mail{$mailid}->{'code'} = 999; }    # Unknown error
            $mail{$mailid}->{'year'} = $year; ### <CJK>###
            $mail{$mailid}->{'mon'} = $mon;
            $mail{$mailid}->{'day'} = $day;
            $mail{$mailid}->{'time'} = $time;
    ...
    # elsif ( /, ctladdr=/ ) {
        ...
        $mailid = $id;
        if ( m/\s+relay=([^\s,]*)[\s,]/ ) { $mail{$id}->{'relay_r'} = $1; } elsif ( m/\s+mailer=local/ ) { $mail{$id}->{'relay_r'} = 'localhost'; }
        if ( /, stat\=Sent/ ) { $mail{$id}->{'code'} = 1; } elsif ( /, stat\=User\s+unknown/ ) { $mail{$id}->{'code'} = 550; } elsif ( /, stat\=Local\s+configuration/ ) { $mail{$id}->{'code'} = 451; } elsif ( /, stat\=Deferred:\s+(\d*)/ ) { $mail{$id}->{'code'} = $1; } else { $mail{$id}->{'code'} = 999; }
        $mail{$mailid}->{'year'} = $year; ### <CJK>###
        $mail{$id}->{'mon'} = $mon;
        $mail{$id}->{'day'} = $day;
        $mail{$id}->{'time'} = $time;
    ...
    # elsif ( /^([^\t]+)\t([^\t]+)\t[^\t]+\t([^\t]+)\t([^\t]+)\t([^\t]+)\t[^\t]+\t([^\t]+)\t([^\t]+)\t([^\t]+)\t[^\t]+\t[^\t]+\t([^\t]+)\t[^\t]+\t[^\t]+\t[^\t]+\t[^\t]+\t[^\t]+\t([^\t]+)\t([^\t]+)/ ) {
        ...
        if ( $code == 1031 ) {
            # This is for external bound mails
            $ok = 1;
    ...
    # elsif ( /starting delivery/ ) {
        ...
        ( $mon, $day, $time, $delivery, $id, $relay_r,
            $to ) = m/^(\w+)\s+(\d+)\s+(\d+:\d+:\d+)\s+.*\s+\d+(?:\.\d+)?\s+starting delivery (\d+):\s+msg\s+(\d+)\s+to\s+(.*)?\s+(.*)$/;
        if ( !$id ) { ( $yea, $mon, $day, $time, $delivery, $id, $relay_r,
            $to ) = m/^(\d+)-(\d+)-(\d+)\s+(\d+:\d+:\d+).*\s+starting delivery (\d+):\s+msg\s+(\d+)\s+to\s+(.*)?\s+(.*)$/; }
        $mailid = $id;
        if ( $relay_r eq 'local' ) { $mail{$id}->{'relay_r'} = 'localhost'; } elsif ( m/\s+relay=([^\s,]*)[\s,]/ ) { $mail{$id}->{'relay_r'} = $1; } elsif ( m/\s+mailer=local/ ) { $mail{$id}->{'relay_r'} = 'localhost'; }
    ...
    # elsif ( /delivery (\d+): (\w+):/ ) {
        ...
        $mailid = $id;
        if ( $code =~ /success/i ) { $mail{$id}->{'code'}->{$delivery} = 1; } elsif ( $code =~ /deferral/i ) { $mail{$id}->{'code'}->{$delivery} = 999; } else { $mail{$id}->{'code'}->{$delivery} = 999; }
    ...
    # elsif ( /end msg (\d+)/ && scalar %{$mail{$1}->{'to'}} ) {
        # If records for mail id are finished and still mails with no delivery status
        ...
        debug( "For id=$id, found a qmail 'end msg' record. This replace 'delivery' record for delivery=" . join( ',',
            keys %{$mail{$id}->{'code'}} ));
    }
    #
    ...
    if ( $mailid ) {
        ...
        # If we can
        if ( $canoutput ) {
            &OutputRecord( $mail{$mailid}->{'year'} ? $mail{$mailid}->{'year'} : $year, $mail{$mailid}->{'mon'},
                $mail{$mailid}->{'day'}, $mail{$mailid}->{'time'}, $mail{$mailid}->{'from'}, $to,
                $mail{$mailid}->{'relay_s'}, $mail{$mailid}->{'relay_r'}, $code, $mail{$mailid}->{'size'},
                $mail{$mailid}->{'forwardto'}, $mail{$mailid}->{'extinfo'} );
            # Delete mail with generic unknown id (This id can by used by another mail)
            if ( $mailid eq '999' ) {
                debug( " Delete mail for id=$mailid", 3 );
                delete $mail{$mailid};
            }
            # Delete delivery instance for id if qmail (qmail can use same id for several mails with multiple delivery) elsif ( $MailType eq 'qmail' ) {

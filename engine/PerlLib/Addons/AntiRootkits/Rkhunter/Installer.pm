...
Addons::AntiRootkits::Rkhunter::Installer - i-MSCP Rkhunter addon installer
...
package Addons::AntiRootkits::Rkhunter::Installer;
...
sub _addCronTask
{
	my $cronFile = iMSCP::File->new(
		'filename' => "$main::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Addons/AntiRootkits/Rkhunter/Cron.pl"
	);
	...
	Servers::cron->factory()->addTask(
		{
			TASKID => 'Addons::AntiRootkits::Rkhunter',
			MINUTE => '@weekly',
			HOUR => '',
			DAY => '',
			MONTH => '',
...
sub _scheduleCheck
{
	if(! -f -s $main::imscpConfig{'RKHUNTER_LOG'}) {
		...
		$rs = execute(
			...
			"$main::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Addons/AntiRootkits/Rkhunter/Cron.pl >/dev/null 2>&1' " .

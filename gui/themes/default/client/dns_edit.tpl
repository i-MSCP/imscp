...
<form name="edit_dns_frm" method="post" action="{ACTION_MODE}">
	<table class="firstColFixed">
		...
		<tr>
			...
			<td>
				<select id="dns_type" onchange="change_dns_type(this.value)" name="type">
					{SELECT_DNS_TYPE}
				</select>
			</td>
		</tr>
		...
		<tr id="tr_dns_ip_address">
			...
			<td>
				<input id="dns_A_address" type="text" onkeypress="return filterChars(event, IPADDRESS);" name="dns_A_address" value="{DNS_ADDRESS}">
			</td>
		</tr>
		<tr id="tr_dns_ip_address_v6">
			<td><label for="dns_AAAA_address">{TR_DNS_IP_ADDRESS_V6}</label></td>
			<td>
				<input id="dns_AAAA_address" type="text" onkeypress="return filterChars(event, IPv6ADDRESS);" name="dns_AAAA_address" value="{DNS_ADDRESS_V6}">
			</td>
		</tr>
		...
		<tr id="tr_dns_srv_ttl">
			...
			<td>
				<input id="dns_srv_ttl" type="text" onkeypress="return filterChars(event, NUMBERS);" name="dns_srv_ttl" value="{DNS_SRV_TTL}">
			</td>
		</tr>
		<tr id="tr_dns_srv_prio">
			<td><label for="dns_srv_prio">{TR_DNS_SRV_PRIO}</label></td>
			<td>
				<input id="dns_srv_prio" type="text" onkeypress="return filterChars(event, NUMBERS);" name="dns_srv_prio" value="{DNS_SRV_PRIO}">
			</td>
		</tr>
		<tr id="tr_dns_srv_weight">
			<td><label for="dns_srv_weight">{TR_DNS_SRV_WEIGHT}</label></td>
			<td>
				<input id="dns_srv_weight" type="text" onkeypress="return filterChars(event, NUMBERS);" name="dns_srv_weight" value="{DNS_SRV_WEIGHT}">
			</td>
		</tr>
		...
		<tr id="tr_dns_srv_port">
			...
			<td>
				<input id="dns_srv_port" type="text" onkeypress="return filterChars(event, NUMBERS);" name="dns_srv_port" value="{DNS_SRV_PORT}">
			</td>
		</tr>
		<tr id="tr_dns_cname">

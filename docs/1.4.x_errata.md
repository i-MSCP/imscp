...
Plugin errors, outside of those raised by the plugins handler (Module::Plugin), are no longer returned to the caller.
That means that an error which has been raised bya plugin will no longer cause a general failure.

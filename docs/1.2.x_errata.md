...
called 'master i-MSCP SQL user', is created. This change intends to solve issues with SQL servers that are
pre-configured to use new authentication plugins such as unix_socket authentication plugin. With such a
configuration, the password for the SQL root user is no set while server package installation. Instead,
the authentication is done through a unix socket. The plugin does a mapping between the local unix user
and the SQL user.

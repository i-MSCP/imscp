...
sub _removeConfig
{
    ...
    if ( $main::execmode eq 'setup' ) {
        ...
        my $filename = basename( $self->{'ftpd'}->{'config'}->{'FTPD_CONF_FILE'} );
    ...
    return 0 unless -d $dirname && -f "$self->{'ftpd'}->{'bkpDir'}/$filename.system";

    iMSCP::File->new( filename => "$self->{'ftpd'}->{'bkpDir'}/$filename.system" )->copyFile(

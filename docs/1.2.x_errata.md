...
### Custom SPF DNS resource records

To fulfit specific requirements, the SPF DNS resource record type as been added to the list of allowed custom DNS
resource records. It is now possible to define custom SPF records using SPF and TXT DNS resource records. Note
also that when a custom SPF DNS resource record is detected (SPF or TXT), the default SPF DNS resource records set
by i-MSCP are automatically removed.

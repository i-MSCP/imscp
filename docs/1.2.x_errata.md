...
Be aware that some plugin are not yet ready for use with this new version, such as the **Mailman** and **OwnDDNS**
plugins which require further works.

Plugin which are known to work with this new version clearly states that they are compatible with i-MSCP version >= 1.2.3.
...
<table>
	<tr>
		<td>
			**iMSCP_Plugin::getConfigPrev()**
 		</td>
 		<td>
 			Allows to retrieve all previous configuration
 		</td>
 	</tr>
 	<tr>
 		<td>
			**iMSCP_Plugin::getConfigPrevParam()**
		<td>
		<td>

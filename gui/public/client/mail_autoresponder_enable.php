...
function client_checkMailAccountOwner($mailAccountId)
{
	...
	$query = '
		...
		AND
			`t1`.`status` = ?
    ';
	$stmt = exec_query($query, array($mailAccountId, $domainProps['domain_id'], 0, $config->ITEM_OK_STATUS));
...
if (customerHasFeature('mail') && (isset($_REQUEST['mail_account_id']) && is_numeric($_REQUEST['mail_account_id']))) {
	...
	if (client_checkMailAccountOwner($mailAccountId)) {
		if (!isset($_POST['mail_account_id'])) {
			...
			$tpl->assign(
				array(
					'TR_PAGE_TITLE' => tr('i-MSCP - Client / Mail accounts / Activate auto-responder'),
					'THEME_CHARSET' => tr('encoding'),
					'ISP_LOGO' => layout_getUserLogo(),
					'TR_ENABLE_MAIL_AUTORESPONDER' => tr('Activate mail auto-responder'),

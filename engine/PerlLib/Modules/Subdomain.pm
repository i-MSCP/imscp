...
=item add( )
 ...
 Schedule change of custom DNS records that belong to parent domain unless
...
sub _loadData
{
    ...
    my $row = $self->{'_conn'}->run( fixup => sub {
        $_->selectrow_hashref(
            "
                SELECT t1.*,
                    t2.domain_name AS user_home, t2.domain_admin_id, t2.domain_mailacc_limit, t2.domain_php, t2.domain_cgi, t2.external_mail,
    ...
    $self->{'_data'} = do {
        ...
        my $phpini = $self->{'_conn'}->run( fixup => sub {
            $_->selectrow_hashref(
                ...
                ( $confLevel eq 'dmn' ? $self->{'domain_id'} : $self->{'subdomain_id'} ), $confLevel
            ) || {};
        } );
        my $haveCert = defined $self->{'certificate'} && -f "$::imscpConfig{'GUI_ROOT_DIR'}/data/certs/$self->{'subdomain_name'}.$self->{'user_home'}.pem";

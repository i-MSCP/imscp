...
class VirtualFileSystem
{
    ...
    /**
     * @var string Virtual file system root directory (relative to domain root
     *             directory)
     */
    protected $rootDir;
     ...
     * Constructor
     *
     * Creates a virtual file system object for the given $user using $rootDir
     * as root directory.
     *
     * @param string $user FTP username. That is, the username that matches with
     *                     the unix user's homedir
     * @param string $rootDir OPTIONAL Root directory of the virtual file system
     *                        (relative to $domain root directory)
     */
     ...
     *
     * @param string $dirname OPTIONAL Directory path inside the virtual file
     *                        system
     * @return array|bool An array of directory entries on success, FALSE on
     *                    failure
     */
    ...
    public function open()
    {
        ...
        if (Registry::get('config')->SERVICES_SSL_ENABLED == 'yes') {
            ...
            if ($this->stream === false) {
                $this->writeLog(
                    "Couldn't connect to FTP server using SSL.", E_USER_NOTICE
                );
            }
        }
        ...
        if (Registry::get('config')->FTPD_PACKAGE != 'Servers::ftpd::vsftpd'
    ...
    protected function createFtpUser()
    {
        try {
            ...
            exec_query(
                '
                    INSERT INTO ftp_users (
                        userid, passwd, uid, gid, shell, homedir, status
                    ) VALUES (
                        ?, ?, ?, ?, ?, ?, ?
                    )
                ',
            ...
            if ($e instanceof DatabaseException && $e->getCode() == 23000) {
                $this->writeLog(
                    'Concurrent FTP connections are not allowed.',
                    E_USER_WARNING
                );
                return false;
            }
            ...
            $this->writeLog(
                sprintf("Couldn't create FTP user: %s", $e->getMessage())
            );
            return false;
        }
    ...
    public function put($file, $content, $transferMode = self::VFS_ASCII)
    {
        ...
        $tmpFile = @tempnam(Registry::get('config')['GUI_ROOT_DIR']
            . '/data/tmp', 'vfs_');

...
Due to a bug found in previous vlogger script version, it is recommended to run the following command to ensure that
there is no process left which leads to a high CPU load:

...
function init_login($eventManager)
{
    ...
    if (iMSCP_Registry::get('config')['BRUTEFORCE']) {
        $bruteforce = new iMSCP_Plugin_Bruteforce(
            iMSCP_Registry::get('pluginManager')
        );
    ...
    $eventManager->registerListener(
        iMSCP_Events::onAuthentication, 'login_credentials', 99
    );

    // Register listener that is responsible to check domain status and expire date
    $eventManager->registerListener(
        iMSCP_Events::onBeforeSetIdentity, 'login_checkDomainAccount'
    );
}
 ...
 *
 * @param iMSCP_Authentication_AuthEvent $authEvent
 */
function login_credentials(iMSCP_Authentication_AuthEvent $authEvent)
{
    ...
    $authEvent->setAuthenticationResult(new iMSCP_Authentication_Result(
        iMSCP_Authentication_Result::SUCCESS, $identity
    ));
}
...
function login_checkDomainAccount($event)
{
    ...
    if ($row['admin_status'] == 'disabled'
        ...
        set_page_message(
            tohtml(tr('Your account has been disabled. Please, contact your reseller.')),
            'error'
        );
        return;
    }
    ...
    if ($row['domain_expires'] > 0 && $row['domain_expires'] < time()) {
        set_page_message(
            tohtml(tr('Your account has expired. Please, contact your reseller.')),
            'error'
        );
        return;
    }

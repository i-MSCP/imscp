...
class iMSCP_VirtualFileSystem
{
     ...
     *
     * Warning! $domain parameter is not sanitized, so this is left as work for the
     * caller.
     *
     ...
     *
     * Get the directory listing of a specified dir, either in short (default) or
     * long mode.
     *
     ...
     *
     * @param string $file VFS file path.
     * @param int $mode VFS transfer mode. Must be either {@link self::VFS_ASCII} or
     *                                     {@link self::VFS_BINARY}.
     * @return boolean TRUE on success, FALSE on failure.
     */
    ...
    protected function _createTmpUser()
    {
        ...
        $stmt = exec_query($query, array(
                                        $user, $password, $stmt->fields['domain_uid'],
                                        $stmt->fields['domain_gid'], $cfg->CMD_SHELL,

...
From now, if you need to known the context of the template when developing a plugin or listener file, you must use the
...
- `domain_fwd`: http vhost for forwarded domains
- `domain_ssl_fwd`: https vhost for forwarded domains
...
### Web folder skeletons (.well-known directory)

The `.well-known` directory has been added to domain, alias and subdomain skeletons. From now, the `.well-known`
directory will be automatically created on new domain, alias or subdomain creation. This direcetory is created inside of
the site document root: `./htdocs/.well-known`.

Note that for existent domains, you have to create that directory manually if required.

...
the [previous errata](1.3.x_errata.md) file from bottom to top.
...
developing a plugin or a listener file, you must use the new server `VHOST_TYPE`
data that is passed to event listeners. For instance, in the
...
The value of the `VHOST_TYPE` runtime parameter can be one of the following:

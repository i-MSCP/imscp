...
sub preinstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdPreInstall', 'apache_itk');
	$self->stop();
...
sub install
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdInstall', 'apache_itk');

	require Servers::httpd::apache_itk::installer;
	my $rs = Servers::httpd::apache_itk::installer->getInstance()->install();
	return $rs if $rs;
...
sub postinstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdPostInstall', 'apache_itk');
	iMSCP::Service->getInstance()->enable($self->{'config'}->{'HTTPD_SNAME'});
	$self->{'eventManager'}->register(
		'beforeSetupRestartServices', sub { push @{$_[0]}, [ sub { $self->start(); }, 'Apache2/ITK httpd server' ]; 0; }
	);
...
sub uninstall
{
	...
	$self->{'eventManager'}->trigger('beforeHttpdUninstall', 'apache_itk');

	require Servers::httpd::apache_itk::uninstaller;
	my $rs = Servers::httpd::apache_itk::uninstaller->getInstance()->uninstall();
	return $rs if $rs;

	$self->{'eventManager'}->trigger('afterHttpdUninstall', 'apache_itk');
	$self->restart();
}
...
sub setEnginePermissions
{
	...
	require Servers::httpd::apache_itk::installer;
	my $rs = Servers::httpd::apache_itk::installer->getInstance()->setEnginePermissions();
	return $rs if $rs;
...
sub addHtuser
{
	...
	$file->save();
	$file->mode(0640);
	$file->owner($main::imscpConfig{'ROOT_USER'}, $data->{'GROUP'});
...
sub buildConfFile
{
	...
	$self->{'eventManager'}->trigger('onLoadTemplate', 'apache_itk', $filename, \my $cfgTpl, $data);
...
sub _addCfg
{
	...
	unless (-f "$self->{'config'}->{'HTTPD_CUSTOM_SITES_DIR'}/$data->{'DOMAIN_NAME'}.conf") {
		my $rs = $self->buildConfFile("$self->{'apacheTplDir'}/custom.conf.tpl", $data, {

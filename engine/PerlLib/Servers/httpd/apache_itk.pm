...
sub registerSetupListeners
{
    ...
    Servers::httpd::apache_itk::installer->getInstance()->registerSetupListeners( $em );
}
...
sub _dmnFolders
{
    ...
    push( @folders, [
        "$self->{'config'}->{'HTTPD_LOG_DIR'}/$data->{'DOMAIN_NAME'}", $::imscpConfig{'ROOT_USER'}, $::imscpConfig{'ROOT_GROUP'}, 0755
    ] );

        ...
        try {
                ...
                if ($itemId == $domainId) {
                    ...
                    if ($stmt->rowCount()) {
                        ...
                        if ($row['external_mail_dns_ids'] != '') {
                            exec_query(
                                '
                                    UPDATE domain_dns SET domain_dns_status = ?
                                    WHERE domain_dns_id IN(' . $row['external_mail_dns_ids'] . ')
                                ',
                                'todelete'
                            );
            ...
            if (!empty($items['alias'])) {
                foreach ((array)$items['alias'] as $itemId) {
                    $stmt = exec_query(
                        '
                            SELECT alias_name, external_mail_dns_ids FROM domain_aliasses WHERE alias_id = ?
                            AND domain_id = ?
                        ',

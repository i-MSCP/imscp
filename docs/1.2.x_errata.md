...
#### Admin level PHP configuration options

The PHP configuration options at the administrator level were removed because they were too much confusing for end-users,
and also because almost not used. From now, if you want limit all customers, you must limit their resellers. For
instance, you can change a PHP configuration option value (e.g. memory_limit) for a reseller without giving him more
permissions on the PHP editor. By doing this, this will override default PHP configuration option value for all its
customers.

#### Cascading permissions - Synchronisation

From now, if you change a PHP configuration option value for a specific reseller, this will also affect all its customers.
For instance, if you change the memory limit of a reseller from 256 to 128, the memory limit of all its customers will be
also updated if they are highter than 128.

The same thing occurs for the PHP editor permissions. For instance, If you change a reseller permissions from 'yes' to
'no', the permission will be also removed its customers.

The rule here is: Customers cannot have highter privileges than their resellers.
...
i-MSCP installation/reconfiguration phase. For instance, when the 'per_site' PHP configuration level is used, a customer
will be able to set different PHP configuration options for its sites. Previously this was not possible because the PHP
configuration options were defined for the first domain only.

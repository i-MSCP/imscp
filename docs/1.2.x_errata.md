...
rebuilt of files which belong to HTTP server implementations. This also allows to process the custom DNS records more
faster than before because from now they are managed by a dedicated module.

...
function _client_getVerifiedData($itemId, $itemType)
{
    ...
    if ($itemType == 'normal') {
        $stmt = exec_query(
            'SELECT domain_id, domain_name AS name, external_mail, external_mail_dns_ids FROM domain WHERE domain_id = ?',
            $domainId
        );
        ...
        $stmt = exec_query(
            '
                SELECT domain_id, alias_name AS name, external_mail, external_mail_dns_ids FROM domain_aliasses
                WHERE alias_id = ? AND domain_id = ?
            '
            ,
...
function client_editExternalMailServerEntries($item)
{
    ...
    if (!empty($_POST)) {
        ...
        if (!empty($verifiedData['external_mail_dns_ids'])) {
            $stmt = execute_query(
                'SELECT * FROM domain_dns WHERE domain_dns_id IN(' . implode(',', $verifiedData['external_mail_dns_ids']) . ')'
            );
            ...
            if ($stmt->rowCount()) {
                ...
                if ($verifiedData['item_type'] == 'normal') {
                    $query = '
                        UPDATE domain SET domain_status = ?,  external_mail = ?, external_mail_dns_ids = ?
                        WHERE domain_id = ?
                    ';
                } else {
                    $query = '
                        UPDATE domain_aliasses SET alias_status = ?, external_mail = ?, external_mail_dns_ids = ?
                        WHERE alias_id = ?
                    ';
                }
            ...
            return;// Only to make some IDE happy
        }
    }
...
function client_generateView($verifiedData, $data)
{
    ...
    $tpl->assign(array(
            ...
            'TR_CANCEL' => tr('Cancel'),
            'TR_UPDATE' => tr('Update'),
            'TR_MX_TYPE_TOOLTIP' => tr('Domain: Setup an external MX host to relay mail of your entire domain.') . '<br><br>' . (
                customerHasFeature('mail') ? tr('Wildcard: Setup an external MX host to relay mail for inexistent subdomains.') . '<br><br>' : ''
            ) .
                tr('Spam Filter: Setup an external MX host to relay mail of your entire domain but retains our server as final mailhost.') . '<br><br>' .

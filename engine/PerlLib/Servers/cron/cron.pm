...
sub preinstall
{
	...
	$self->{'eventManager'}->trigger('beforeCronPreinstall', 'cron');
...
sub install
{
	...
	$self->{'eventManager'}->trigger('beforeCronInstall', 'cron');
	$self->{'eventManager'}->trigger('onLoadTemplate', 'cron', 'imscp', \my $cfgTpl, { });

	$cfgTpl = iMSCP::File->new( filename => "$self->{'cfgDir'}/imscp" )->get() unless defined $cfgTpl;
	$cfgTpl = process(
		{
	...
	$file->mode(0644);
	$file->copyFile("$self->{'config'}->{'CRON_D_DIR'}/imscp");
...
sub postinstall
{
	...
	$self->{'eventManager'}->trigger('beforeCronPostInstall', 'cron');
...
sub addTask
{
	...
	exists $data->{'COMMAND'} && exists $data->{'TASKID'} or die('Missing command or task ID');

	$filepath ||= "$self->{'config'}->{'CRON_D_DIR'}/imscp";
	-f $filepath or die(sprintf('Could not add cron task: %s not found.', $filepath));
	...
	$self->{'eventManager'}->trigger('beforeCronAddTask', \$wrkFileContent, $data);
...
sub deleteTask
{
	...
	$data = { } unless ref $data eq 'HASH';
	exists $data->{'TASKID'} or die('Missing task ID');

	$filepath ||= "$self->{'config'}->{'CRON_D_DIR'}/imscp";
	-f $filepath or die(sprintf('Could not remove cron task: %s not found.', $filepath));

	my $filename = fileparse($filepath);
	my $wrkFileContent = iMSCP::File->new( filename => $filepath )->get;

	$self->{'eventManager'}->trigger('beforeCronDelTask', \$wrkFileContent, $data);
	$wrkFileContent = replaceBloc(
	...
	$self->{'eventManager'}->trigger('afterCronDelTask', \$wrkFileContent, $data);
...
=item _init()
 ...
 Return Servers::cron::cron, die on failure

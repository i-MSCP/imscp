...
foreach ($bookmarksdata as $bookfoo) {
  if (($bookfoo["url"] != '') and ($bookfoo["last_visit"] > 0)) {
    $bycount[$n]["last_visit"] = $bookfoo["last_visit"];
...
echo '<td align="right"><font color="' . $color[1] . '"><a href="bookmarks_bycount.php">Most Visits</a></font></td>';
echo '</tr></table>';
...
foreach ($bycount as $bookfoo) {
  echo '&bull;&nbsp;(';
	echo strftime("%d-%b-%Y", $bookfoo["last_visit"]);

...
{
    return $autoinstallerAdapterInstance
        if defined $autoinstallerAdapterInstance;

    my $distribution = iMSCP::LsbRelease->getInstance()->getId( TRUE );
    ...
    die( sprintf(
        "Couldn't instantiate %s autoinstaller adapter: %s", $distribution, $@
    )) if $@;

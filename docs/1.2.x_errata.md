...
This new parameters allows the administrator to disable bind mounts of customers httpd log directories. To disable
...
Be aware that when bind mount of httpd log directories is disabled, the 'logs' directory located in the home directory
...
This script has been renamed to imscp-reconfigure.

### Parameters

### i-MSCP master SQL user

Starting with this new version, usage of default SQL root user is prohibited. Instead, a dedicated SQL user,
called 'master i-MSCP SQL user', will be created. This change intends to solve issues with SQL servers that
are pre-configured to use new authentication plugins such as unix_socket authentication plugin. With such a
configuration, the password for the SQL root user is no defined while server package installation. Instead,
the authentication is done through an unix socket which operates a kind of mapping between local unix user
and SQL user.

Note that you'll be still asked for the SQL root password when needed (case of a remote SQL server, or when a
password is required for the SQL root user). However, the SQL root user will be used only while installation
or reconfiguration.
...
i-MSCP will fail to bind mount customer log directories in their homedirs. You can achieve this task as follow:
...
You should also restart the imscp_mountall service inside the LXC container:
...
#### DKIM/DMARC TXT DNS resource record

It is now possible to add DKIM/DMARC TXT DNS resource records.

#### SPF DNS resource record

To fulfit specific requirements, the SPF DNS resource record type as been added to the list of allowed custom DNS
resource records. It is now possible to define custom SPF records using SPF and TXT DNS resource records. However
...
speficy a values lower than 60 seconds.
